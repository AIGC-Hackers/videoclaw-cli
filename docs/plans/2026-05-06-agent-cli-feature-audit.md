...
| F9.3 git status 干净 | ⚠ | 有两个 untracked：`.gsd-id` `mcp-shim/uv.lock` —— 已知，需要 .gitignore |
...
**差距 G7**：F9.3 把 `.gsd-id` `mcp-shim/uv.lock` 加进 `.gitignore`（或确认它们应该被追踪）
...
| **G7**  | F9.3   | P2 | .gitignore 添加 .gsd-id / mcp-shim/uv.lock | 1 |
